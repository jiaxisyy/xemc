...
unsigned char gau8_X[X_RANGE];      // X位元件定义
unsigned char gau8_T[T_RANGE];      // T位元件定义
unsigned char gau8_C[C_RANGE];      // C位元件定义

unsigned char gau8_Y[Y_RANGE];      // Y位元件定义
unsigned char gau8Wt_Y[Y_RANGE];    // Y位元件定义
unsigned char gau8_ChY[Y_RANGE];

unsigned char gau8_M[M_RANGE];      // M位元件定义
unsigned char gau8Wt_M[M_RANGE];    // M位元件定义
unsigned char gau8_ChM[M_RANGE];

unsigned char gau8_S[S_RANGE];      // S位元件定义
unsigned char gau8Wt_S[S_RANGE];    // S位元件定义
unsigned char gau8_ChS[S_RANGE];

unsigned char gau8_SM[SM_RANGE];    // SM位元件定义
unsigned char gau8Wt_SM[SM_RANGE];  // SM位元件定义
unsigned char gau8_ChSm[SM_RANGE];

unsigned short gau16_D[D_RANGE];        // D字元件定义
unsigned short gau16Wt_D[D_RANGE];      // D字元件定义
unsigned char gau16_ChD[D_RANGE];

unsigned short gau16_R[R_RANGE];        // R字元件定义
unsigned short gau16Wt_R[R_RANGE];      // R字元件定义
unsigned char gau16_ChR[R_RANGE];

unsigned short gau16_SD[SD_RANGE];      // SD字元件定义
unsigned short gau16Wt_SD[SD_RANGE];    // SD字元件定义
unsigned char gau16_ChSd[SD_RANGE];

unsigned short gau16_Z[Z_RANGE];        // Z字元件定义
unsigned short gau16Wt_Z[Z_RANGE];      // Z字元件定义
unsigned char gau16_ChZ[Z_RANGE];

unsigned short gau16_TCurValue[T_RANGE];// 定时器的当前值
unsigned long gau32_CCurValue[C_RANGE]; // 计数器的当前值(分三段)


unsigned long gu32_CurMsTime;
...
const unsigned short mau16_Crc16Tab[256] =
{
...
unsigned short Crc_Check(unsigned char *data, unsigned short length)
{
    ...
    while (length--) {
        ...
        uchCRCLo = uchCRCHi ^ (mau16_Crc16Tab[uIndex]>>8) ;
        uchCRCHi = mau16_Crc16Tab[uIndex]&0xff ;
    }
...
{
    unsigned short Clc_Crc;

    if(len ==0) {
        return 0;
    }
    
    Clc_Crc = Crc_Check(pu8Dta, len);
    pu8Dta[len] = (unsigned char)(Clc_Crc >>8);
    pu8Dta[len +1] = (unsigned char)(Clc_Crc);
    len += 2;
    return len;
}
